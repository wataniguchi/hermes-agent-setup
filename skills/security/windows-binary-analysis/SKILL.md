...
- **A string of `exec`/`clone` 500 errors is not, by itself, evidence the VM is broken — it is very often a transient network blip between the bridge and Proxmox, not a VM-level problem.** The bridge retries transient failures internally now, but if you still see one, do not conclude the VM is corrupted and destroy it. First retry the exact same command once or twice yourself; if it starts succeeding, the earlier failures were transient and nothing about the VM needs fixing. Only consider the session genuinely dead if the failure is persistent and specific (e.g. a `404`/"config file does not exist" for the vmid you're actually using right now — not for some other vmid you destroyed earlier in the same task, which will correctly and expectedly 404 forever).
- **Never state an infrastructure diagnosis ("the bridge is down," "the template is missing," "the VM config doesn't exist") without checking it against the actual evidence you have.** If the bridge's health endpoint returns `ok`, and earlier calls in this same session succeeded, do not report the infrastructure itself as broken — look for a more specific, evidence-supported explanation (e.g. a transient network error, or an error referencing a vmid you yourself destroyed) before concluding something is globally wrong.
- **Never target vmid 9000 (or whatever `PVE_TEMPLATE_VMID` is) directly with any operation.** This is the golden template every analysis session is cloned from — not something to `exec`, `start`, `stop`, push files to, or destroy. The bridge itself now refuses any such call with a `403` regardless of what's requested, so this can't actually cause damage — but if you see that error, it means you have the wrong vmid; use the one returned by `start`, not the template.
